...
class Poll extends PollActiveRecord
{
    ...
    {
        return (new Query())->from('forum.podium_poll_vote')->where([
            'poll_id' => $this->id,
            'caster_id' => $userId
        ])->count('id') ? true : false;
    }
        ...
        if (!empty($votes)) {
            ...
            try {
                if (!Podium::getInstance()->db->createCommand()->batchInsert(
                        'forum.podium_poll_vote', ['poll_id', 'answer_id', 'caster_id', 'created_at'], $votes
                    )->execute()) {
    ...
    public function podiumDelete()
    {
        $transaction = static::getDb()->beginTransaction();
        try {
            if (!Podium::getInstance()->db->createCommand()->delete('forum.podium_poll_vote', ['poll_id' => $this->id])->execute()) {
